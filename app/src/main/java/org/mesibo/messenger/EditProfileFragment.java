...
public class EditProfileFragment extends Fragment implements MediaPicker.ImageEditorListener, MesiboProfile.Listener {
    ...
    private LinearLayout registerProfileSave;
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        Objects.requireNonNull(getActivity()).getMenuInflater().inflate(R.menu.edit_profile_menu, menu);
        //mScreen.menu = menu;
        MenuItem.OnMenuItemClickListener menuhandler = item -> {
            save();
            return true;
        };
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        registerProfileSave = (LinearLayout) v.findViewById(R.id.register_profile_save);
        setUserPicture();

        mProfileImage.setOnClickListener(v1 -> UIManager.launchImageViewer(getActivity(), getProfile().getImage().getImagePath()));
        registerProfileSave.setOnClickListener(v1 -> {
            StartUpActivity.newInstance(getContext(), true);
        });

        ImageView mProfileButton = (ImageView) v.findViewById(R.id.edit_user_image);
        mProfileButton.setOnClickListener(v12 -> {
            MenuBuilder menuBuilder = new MenuBuilder(getActivity());
            MenuInflater inflater1 = new MenuInflater(getActivity());
            inflater1.inflate(R.menu.image_source_menu, menuBuilder);
            MenuPopupHelper optionsMenu = new MenuPopupHelper(getActivity(), menuBuilder, v12);
            optionsMenu.setForceShowIcon(true);
            menuBuilder.setCallback(new MenuBuilder.Callback() {
                @Override
                public boolean onMenuItemSelected(MenuBuilder menu, MenuItem item) {
                    if (item.getItemId() == R.id.popup_camera) {
                        if (AppUtils.acquireUserPermission(getActivity(), Manifest.permission.CAMERA, CAMERA_PERMISSION_CODE)) {
                            MediaPicker.launchPicker(getActivity(), MediaPicker.TYPE_CAMERAIMAGE);
                        }
                        return true;
                        ...
                        MesiboProfile profile1 = getProfile();
                        profile1.deleteImages();
                        profile1.setImage(null);
                        profile1.save();
                        return true;
                    }
                    return false;
